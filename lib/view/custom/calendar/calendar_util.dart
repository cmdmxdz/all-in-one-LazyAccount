...
class CalendarUtil {
  ...
  DateTime getActualDate(int index, DateTime currentTime) {
    if (calendarShow == CalendarShow.CalendarShowMonth) {
      ...
      DateTime minDateTime =
          DateTime(this.initTime.year, this.initTime.month - month);
      DateTime maxDateTime =
          DateTime(this.initTime.year, this.initTime.month - month + 1);

      if (!currentTime.isBefore(minDateTime) &&
          currentTime.isBefore(maxDateTime)) {
        return currentTime;
      }
      return minDateTime;
    } else {
      DateTime weekDate =
          this.initTime.add(Duration(days: 7 * (index - _kInitIndex)));
      ...
      if (!currentTime.isBefore(minWeekDate) &&
          currentTime.isBefore(maxWeekDate)) {
        return currentTime;
      }
      return minWeekDate;
    }
  ...
  int _weekIndex(DateTime dateTime) {
    var nowFirstDate = this.initTime.add(Duration(
        days: -((this.initTime.weekday - (this.firstDayOfWeek % 7)) % 7)));
  ...
  static double aspectRatio(double childAspectRatio, int firstDayOfWeek,
      DateTime dateTime, CalendarShow show) {
    if (show == CalendarShow.CalendarShowMonth) {
      var rowCount = getRowCount(dateTime.year, dateTime.month, firstDayOfWeek);
      return (7.0 / rowCount * childAspectRatio);
    } else {
  ...
  static int getCurrentRow(int firstDayOfWeek, DateTime currentTime) {
    var firstWeekDay =
        DateTime(currentTime.year, currentTime.month, 1).weekday % 7;
